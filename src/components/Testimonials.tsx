    ...
    <section className="bg-[#0f0f1a] py-20 text-white">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-4 text-slate-300/80">Ordagrant utdrag från Bokadirekt och officiell webbplats.</p>
        </div>
          ...
          {business.testimonials.map((review, index) => (
            <figure key={index} className="rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur">
              ...
              <figcaption className="mt-4 text-xs text-slate-300/60">
                — {review.author} ·{" "}
                <a href={review.sourceUrl} target="_blank" rel="noopener noreferrer" className="underline hover:text-slate-100">{review.source}</a>
